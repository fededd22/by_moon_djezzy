...
def api_available_offers():
    r = djezzy(g.cid, "GET", f"/api/v1/subscribers/available-offers/{g.phone}")
    if r is None:
        return upstream_failed(g.cid)
    if r.status_code != 200:
        return fail(f"فشل جلب العروض ({r.status_code})", 502)
    try:
        offers = r.json().get("data", {}).get("offers", []) or []
    except Exception:
        return fail("ردّ غير مفهوم من الخادم", 502)
    return ok(offers=[
        {"name": str(o.get("name", "?")), "price": str(o.get("price", "?")), "duration": str(o.get("duration", "?"))}
        for o in offers[:15] if isinstance(o, dict)
    ])
